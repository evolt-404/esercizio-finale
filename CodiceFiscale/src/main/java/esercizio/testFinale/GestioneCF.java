...
public class GestioneCF {
	...
	public  CodiceFiscale calcoloCodiceFiscale(String cognome, String nome, String dataDiNascita, String comuneDiNascita, String sesso) {
		...
		for (int i = 0; i < cognome.length(); i++) {
			if (cont == 3)
				break;
			if (cognome.charAt(i) != 'A' && cognome.charAt(i) != 'E' && cognome.charAt(i) != 'I'
		...
		//ANNO
		codFis += dataDiNascita.substring(8, 10);
		//MESE
		int mese = 0;
		if (dataDiNascita.charAt(3) == '0')
			mese = Integer.parseInt(dataDiNascita.substring(4, 5));
		else
			mese = Integer.parseInt(dataDiNascita.substring(3, 5));
		switch (mese) {
		case 1: {
			codFis += "A";
			break;
		...
		codiceFiscale.setDataDiNascita(dataDiNascita);
		codiceFiscale.setComuneDiNascita(comuneDiNascita);
